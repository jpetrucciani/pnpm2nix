...
let
  ...
  indexOf =
    ...
    in
    (elem: list: _indexOf elem list 0);
  ...
  injectNameVersionAttrs = packageSet:
    ...
    in
    lib.foldl addAttrs packageSet (lib.attrNames packageSet);

  # Resolve peer dependencies to pnpmlock attribute names
  #
  # packageSet: packages from pnpmlock.yaml
  resolvePackagePeerDependencies = packageSet:
    let
      ...
      resolve = pname: versionSpec: with builtins; let
        ...
        matches = lib.filterAttrs
          (n: v:
            semver.satisfies v.version versionSpec)
          nameMatches;
        matchPairs = lib.mapAttrsToList
          (name: value:
            { inherit name; version = value.version; })
          matches;
        sorted = builtins.sort
          (a: b:
            lib.versionOlder b.version a.version)
          matchPairs;
        hasMatch = lib.length sorted > 0;
        finalMatch = lib.elemAt sorted 0;
      in
      if hasMatch then finalMatch.name else null;

      rewriteAttrSet = (acc: pkgAttr: acc // (
        let
          pkg = acc."${pkgAttr}";
          attrSet =
            if (lib.hasAttr "peerDependencies" pkg)
            then pkg.peerDependencies
            else { };

          peerDependencies = lib.foldl
            (acc: pkgAttr: (
              let
    ...
    in
    lib.foldl rewriteAttrSet packageSet (lib.attrNames packageSet);

  # Find the attribute name for a pnpmlock package
  findAttrName = attrSet: depName: depVersion:
    let
      slashed = "/${depName}/${depVersion}";
    in
    if (lib.hasAttr slashed attrSet) then slashed else depVersion;

  # Resolve dependencies to pnpmlock attribute names
  #
  # packageSet: packages from pnpmlock.yaml
  resolvePackageDependencies = packageSet:
    let
  ...
  resolveDependencies = pnpmlock:
    let
      ...
      rewriteAttrs = attr:
        let
          attrSet =
            if (lib.hasAttr attr pnpmlock && pnpmlock."${attr}" != null)
            then pnpmlock."${attr}"
            else { };
        in
        lib.mapAttrsToList
          (depName: depVersion:
            (findAttrName packageSet depName depVersion))
          attrSet;
  ...
  # Something something
  breakCircular = dependencyAttributes: packageSet:
    let
      ...
      walkGraph = (pkgAttr: visitStack: acc:
        let
        ...
        in
        if hasDeps then reducedNodes else rewrittenSet);

    in
    lib.foldl
      (acc: attrName: acc //
        (walkGraph attrName [ ] acc))
      packageSet
      nonLocalDependencyAttrs;
...
in
(pnpmlockYAML: rewriteGraph pnpmlockYAML)
