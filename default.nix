  ...
  mkPnpmPackage =
    ...
    let
        ...
        let
          ...
          localPackages =
            ...
            lib.mapAttrs
              ...
              resolvedSpecifiers;
        in
        nonLocalPackages // localPackages;

      # Wrap sources in a directory named the same as the node_modules/ path
      wrapRawSrc = src: pname: (stdenv.mkDerivation (
        let
      ...
      wrapSrc = pkgInfo:
        ...
        in
        wrapRawSrc src pkgInfo.pname;

      mkPnpmModule = pkgInfo:
        let
          ...
          deps = builtins.map (attrName: packages."${attrName}")
            ...
            (lib.unique (lib.flatten (builtins.map
              (attr: pnpmlock.packages."${attr}".dependencies)
              pkgInfo.constituents)));
    ...
    assert (pnpmlock.lockfileVersion >= 5 || pnpmlock.lockfileVersion < 6.0);
    (mkPnpmDerivation {
      deps = (builtins.map
